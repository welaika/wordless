...
class Diff{
   ...
   * $ignoreIndent      - false to compare also white spaces in the beginning
   *                      on the line, true to ignore them
   */
  public static function compare(
      $string1, $string2, $compareCharacters = false, $ignoreIndent = true){
    ...
    while ($start <= $end1 && $start <= $end2
        && static::compareLine($sequence1[$start], $sequence2[$start], $ignoreIndent)){
      $start ++;
    }

    // skip any common suffix
    while ($end1 >= $start && $end2 >= $start
        && static::compareLine($sequence1[$end1], $sequence2[$end2], $ignoreIndent)){
      $end1 --;
      $end2 --;
    }
    ...
    $partialDiff =
        self::generatePartialDiff($table, $sequence1, $sequence2, $start, $ignoreIndent);
  ...
  private static function computeTable(
      $sequence1, $sequence2, $start, $end1, $end2, $ignoreIndent = true){
    ...
    for ($index1 = 1; $index1 <= $length1; $index1 ++){
      ...
      for ($index2 = 1; $index2 <= $length2; $index2 ++){
        ...
        if (static::compareLine($sequence1[$index1 + $start - 1],
            $sequence2[$index2 + $start - 1], $ignoreIndent)){
  ...
  private static function generatePartialDiff(
      $table, $sequence1, $sequence2, $start, $ignoreIndent = true){
    ...
    while ($index1 > 0 || $index2 > 0){
      ...
      if ($index1 > 0 && $index2 > 0
          && static::compareLine($sequence1[$index1 + $start - 1],
              $sequence2[$index2 + $start - 1], $ignoreIndent)){
        ...
        $index2 --;
          ...
          && static::compareLine($table[$index1][$index2], $table[$index1][$index2 - 1], $ignoreIndent)){
