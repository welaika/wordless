...
class MediaHelper {
	...
	function resize_image( $src, $width, $height ) {
		...
		if ( ! file_exists( $img_filename ) ) {
			...
			} elseif ( $height_orig > $height && $width_orig > $width ) { // if both bigger => scale
				$to_scale   = 1;
			...
			} elseif ( ( $width == $width_orig && $height_orig > $height ) || ( $height == $height_orig && $width_orig > $width ) ) {// one equal one bigger
				$to_crop = 1;
			} else { // some problem...
				echo 'ALARM';
			}
