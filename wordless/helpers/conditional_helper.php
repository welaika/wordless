	...
	*/
	function is_page_wpml( $page_title = array() ) {
		...
		foreach ( $pages as $page ) {
            $pageObj = get_page_by_title( $page ); //@codingStandardsIgnoreLine
			$icl_object_id = array();
			$icl_object_id = icl_object_id( $pageObj->ID, 'page', true ); //@codingStandardsIgnoreLine

            if ( is_page( array( $icl_object_id, $pageObj->ID ) ) ) //@codingStandardsIgnoreLine
				return true;
		}
