	...
	*/
	function truncate( $text, $options = array() ) {
		...
		if ( $options['word_count'] ) {
			...
			if ( $options['html'] ) {
                $actual_error_reporting_level = error_reporting(); //@codingStandardsIgnoreLine
                error_reporting(0); //@codingStandardsIgnoreLine
				$doc = new DOMDocument();
                libxml_use_internal_errors(true); //@codingStandardsIgnoreLine
				$doc->loadHTML( '<meta http-equiv="Content-Type" content="text/html; charset=utf-8">' . "<wrapper>{$text}</wrapper>" );
                error_reporting($actual_error_reporting_level); //@codingStandardsIgnoreLine
				libxml_clear_errors();
